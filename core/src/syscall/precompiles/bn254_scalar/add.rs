...
const OP: FieldOperation = FieldOperation::Add;

#[derive(Debug, Clone, AlignedBorrow)]
#[repr(C)]
pub struct Bn254ScalarAddCols<T> {
    is_real: T,
    shard: T,
    clk: T,
    p_ptr: T,
...
pub struct Bn254ScalarAddChip;

impl Bn254ScalarAddChip {
    pub fn new() -> Self {
        Self
    }
}

impl Syscall for Bn254ScalarAddChip {
    fn execute(
        &self,
...
impl<F: PrimeField32> MachineAir<F> for Bn254ScalarAddChip {
    type Record = ExecutionRecord;

    type Program = Program;

    fn name(&self) -> String {
        "Bn254ScalarAdd".to_string()
    }
    ...
    fn generate_trace(&self, input: &Self::Record, output: &mut Self::Record) -> RowMajorMatrix<F> {
            ...
            .filter(|e| e.op == OP)
        {
            ...
            let cols: &mut Bn254ScalarAddCols<F> = row.as_mut_slice().borrow_mut();
...
impl<F: Field> BaseAir<F> for Bn254ScalarAddChip {
    fn width(&self) -> usize {
        NUM_COLS
    }
}

impl<AB> Air<AB> for Bn254ScalarAddChip
where
    AB: SP1AirBuilder,
    // AB::Expr: Copy,
{
    fn eval(&self, builder: &mut AB) {
        ...
        let row: &Bn254ScalarAddCols<AB::Var> = (*row).borrow();
        ...
        let syscall_id = AB::F::from_canonical_u32(SyscallCode::BN254_SCALAR_ADD.syscall_id());
